...
-- =========================================================
--             AUTO MELEE NO LADRÃO DURANTE O ROUBO
-- =========================================================
-- Envia o Player do ladrão ao RemoteEvent MeleeHit em loop
-- enquanto o evento de roubo estiver ativo. O loop recebe um
-- identificador próprio para garantir que um loop antigo nunca
-- continue depois que o roubo terminar ou trocar de alvo.

local meleeLoopAtivo = false
local meleeAlvo = nil
local meleeLoopId = 0
local MELEE_INTERVALO = 0.10

local function pararMeleeContraLadrao()

	meleeLoopAtivo = false
	meleeAlvo = nil
	meleeLoopId += 1

end

local function iniciarMeleeContraLadrao(jogador)
	...
	if jogador.Parent ~= Players then
		return
	end
	...
	local meuLoopId = meleeLoopId

	task.spawn(function()

		while meleeLoopAtivo
			and alertaAtivo
			and meleeLoopId == meuLoopId
		do

			local alvo = meleeAlvo

			if not alvo
				or alvo.Parent ~= Players
				or alvo == LocalPlayer then
				break
			end

			pcall(function()
				MeleeHit:FireServer(alvo)
			end)

			task.wait(MELEE_INTERVALO)

		end

		if meleeLoopId == meuLoopId then
			meleeLoopAtivo = false
			meleeAlvo = nil
		end
...
jan:add(HackEvent.OnClientEvent:Connect(function(data)
	...
	if kind == "phase" then
		...
		if role == "victim" then
			...
			iniciarMeleeContraLadrao(ladrao)
